...
function Settings() {
  ...
  const handleUsernameUpdate = () => {
    ...
    if(proceed) {
        axios.put('https://video-noting-web-app-80f672477ea7.herokuapp.com/auth',{username: newUsername}, {headers: {accessToken: localStorage.getItem("accessToken")}})
        .then((response) => {
  ...
  const hadlePasswordUpdate = (event) => {
    ...
    if(proceed) {
      axios.post("https://video-noting-web-app-80f672477ea7.herokuapp.com/auth/update-password", {currentPassword: currentPassword, newPassword: newPassword}, {headers: {accessToken: localStorage.getItem("accessToken")}}).then((response) => {
        if(response.data.error) {
          alert(response.data.error)
        }else {
          alert(response.data.message)
  ...
  const handleDelete = () => {
    ...
    if(proceed) {
      ...
      axios.delete("https://video-noting-web-app-80f672477ea7.herokuapp.com/upload-image", { headers: { accessToken } })
      .then((response) => {
        if(!response.data.error) {
          console.log("no error")
          axios.delete("https://video-noting-web-app-80f672477ea7.herokuapp.com/auth/delete-account", {headers: {accessToken: accessToken}})
          .then((response) => {
            if(response.data.error) {
              alert(response.data.error)
            }else {
  ...
  const uploadImage = () => {
    ...
    const upload = () => {
      ...
      axios.post("https://video-noting-web-app-80f672477ea7.herokuapp.com/upload-image", formData, {headers: {accessToken: accessToken}}).then((response) => {
