...
function Folders() {
  ...
  useEffect(() => {
    ...
    axios.get("http://localhost:3001/folders", {headers: {accessToken: localStorage.getItem('accessToken')}})
    .then((response) => {
  ...
  const handleDelete = (id) => {
    ...
    if(proceed) {
      ...
      axios.delete(`http://localhost:3001/folders/${id}`, {headers: {accessToken: localStorage.getItem("accessToken")}})
      .then((response) => {
